...
for t = Dati.dt : Dati.dt : Dati.T - Dati.dt
   ...
   %b_nbc = (M_nbc-0.5*Dati.dt^2*A_nbc-0.5*Dati.dt^2*C_nbc)*u0+(Dati.dt*M_nbc-0.5*Dati.dt^2*D_nbc)*v0+0.5*Dati.dt^2*b_nbc;
    %%NUOVO SCHEMA
    b_nbc = (Dati.dt*D_nbc - M_nbc)*u0+(2*M_nbc-Dati.dt*D_nbc-Dati.dt^2*(A_nbc+C_nbc))*u1+Dati.dt^2*b_nbc;
    if (strcmp(Dati.bc,'NN'))
        u2 = M_nbc\b_nbc;
